...
Files changed: assets/css/mobile-ux.css ONLY. No HTML was modified. (mobile-ux.css is linked by all 8 main pages, so one patch governs the whole site. 50 files in → 50 out, pure edit.)

Typography variables added (in :root, mobile-ux.css):
- --acl-font: "Tajawal","IBM Plex Sans Arabic","Noto Kufi Arabic","Segoe UI",system-ui,-apple-system,BlinkMacSystemFont,Arial,sans-serif (applied to body to consolidate the stack site-wide).
- --fs-hero-title: clamp(2.2rem, 4.2vw, 4rem) — the calmer scale recommended for long Arabic titles.
- --fs-hero-lead: clamp(1.05rem, 1.45vw, 1.35rem)
- --fs-section-title: clamp(1.65rem, 3vw, 2.6rem); --fs-card-title: clamp(1.2rem, 2vw, 1.55rem); --fs-body: clamp(1rem,1vw,1.1rem); --fs-small: .92rem
- line-height tokens: --lh-body 1.9, --lh-lead 2, --lh-card 1.8, --lh-nav 1.2, --lh-title 1.2

Title sizes normalized: replaced the v41 hero block (clamp up to 4.35rem) with the unified token. Hero/page/article/section/wisdom/wheel/calendar/search titles all use --fs-hero-title; base h1 also maps to it (no !important) so unclassed titles like the 404 page match. Mobile (≤760px) caps titles at clamp(1.9rem, 7.4vw, 2.5rem) so they never fill the screen. Result: one title size per width across all 8 pages — 390px = 32.2px, 768px = 37.3px, 1366px = 57.4px (was two-to-three different sizes before). Career-OS mobile title dropped from ~230px/5 lines to ~161px/4 lines; desktop is contained and calm.

Subtitle sizes normalized: hero leads/intros unified to --fs-hero-lead with line-height 2 for comfortable Arabic reading. Eyebrow uppercase English keeps .14em tracking; Arabic titles use only a slight -.01em (no stretching, no wide tracking).

Hierarchy preserved on every page: eyebrow/label → main title → subtitle/lead → buttons/cards, with no page's title dominating. Hero titles get max-width:22ch on desktop for elegant wrapping, 100% on mobile.

Navigation: unchanged — same 9 labels, same order (Home · Career-OS · Book Tools · Wisdom · Life Calendar · Year Wheel · Water Wonder · Search · Contact), single line on desktop (~34–36px tall), readable mobile menu. nav line-height kept at 1.2.

Pages tested at 390/430/768/1366px: index, career-os (+ #book-derived-tools), wisdom-of-day, life-operating-calendar, year-operating-wheel, water-without-fear, search, 404. Verified: consistent title size, consistent subtitle size, no giant hero titles, no horizontal overflow (0px), unified nav intact, mobile menu open/close works, search works (English + Arabic), JS clean (node --check on all three JS files), all internal links/anchors resolve, sitemap valid.

Preserved: Water Wonder, Search page, Book Tools 001–010 (10 modules), Wisdom, Life Calendar, Year Wheel, all 3 PDFs, both emails (index.html#contact).

Safety checks passed: no password / invite gate (no access-pass/invite-pass/SITE_ACCESS_PASS/SITE_INVITE_PASS/acl-gate/localStorage gate in any HTML/JS/CSS); the four blocked terms absent from every file; no content meaning changed (CSS-only). GitHub/Vercel ready.

## v41 — Navigation and heading consistency patch

- Added a small CSS-only consistency patch in `assets/css/mobile-ux.css`.
- Desktop top navigation now stays on one line across main pages by widening the header wrap, preventing flex wrapping, reducing nav pill padding slightly, and allowing safe horizontal overflow handling inside the nav only.
- Hero/page H1 sizes are now capped with one shared clamp so Arabic titles remain consistent and no longer appear oversized on Career-OS / Book Tools sections.
- No content changed, no pages removed, no password/invite gate added, no search logic changed.

## v40 — Final QA audit + safe polish (search, nav, mobile) · no password

Approach: audit-first. The whole site was driven through headless-Chromium tests at 390/430/768/1366px and a static index/link validator before any edit. v39 was already strong: mobile menu works on all pages, no horizontal overflow anywhere, no console errors, the static search returns results for all 14 sample queries (English + Arabic), and every one of the 148 search-index targets resolves to a real page/anchor. Only three genuine issues were found and fixed; nothing else was touched.

Files changed:
1. All 8 main pages (index, career-os, wisdom-of-day, life-operating-calendar, year-operating-wheel, water-without-fear, search, 404) — nav reorder only. The Search link sat at position 4; the spec requires it at position 8, between Water Wonder and Contact. Reordered to the exact required sequence: Home · Career-OS · Book Tools · Wisdom · Life Calendar · Year Wheel · Water Wonder · Search · Contact. The menu is now byte-identical across all 8 pages (single MD5), same English labels, same order, same links. No other nav change.
2. search.html — accessibility: the search input had no associated label (only the parent container carried an aria-label). Added a visually-hidden <label for="siteSearchInput"> plus a matching aria-label so the field has a programmatic name for screen readers. .visually-hidden is already defined in mobile-ux.css (loaded here), so no new CSS.
3. search.html — SEO: the meta description was mistakenly copied from the Wisdom page ("حكمة اليوم: محرك ثابت..."). Replaced with an accurate description of the search page (bilingual static search across the site's sections). No other meta touched; OG/robots were already correct.

Search checks (all pass): static, no backend, no external libraries, no tracking. Input visible and mobile-readable; suggestion chips work. Empty query handled gracefully; no-results state shows a helpful Arabic message. Arabic normalization in site-search.js folds hamza/alef/ya/ta-marbuta and strips diacritics, so العربية + English both match. Sample queries returned results — water/Water Wonder/chemistry/Book Tools/الماء = 48; hydrology/Year Wheel/Wisdom/Life Calendar/الهيدرولوجي = 14–34; تقويم 40; العجلة 22; الكيمياء 24; الحكمة 4. Index covers Home, Career-OS, Book Tools, Wisdom, Life Calendar, Year Wheel, Water Wonder. Validated: 0 broken targets across 148 entries, 0 gate/private/password content in the index, search.html present in sitemap.xml and in nav, good title + (now correct) meta description.

Navigation checks: unified English nav on all main pages; same order/labels/links everywhere; current-page highlight via aria-current still works (mobile-ux.js); no raw bullets (list-style:none); styled toggle (not a default button).

Mobile-menu checks: on every page at 390px — Menu button visible, tap opens (display:flex, aria-expanded=true), tap again closes (display:none, aria-expanded=false), no double-binding, no open-and-close on one tap, no horizontal overflow. (Logic is the hardened, idempotent initMobileNav from v38; unchanged.)

JS / asset / path fixes: none needed. main.js, mobile-ux.js, and site-search.js all pass node --check; assets load on every page; relative asset paths already resolve correctly on flat GitHub/Vercel deployment (verified over a real HTTP server, not just file://), so paths were intentionally left as-is per "do not over-edit paths that already work."

Pages tested: index, career-os (+ #book-derived-tools jump and Book Tools index), wisdom-of-day (daily card renders, copy + show-another + 60-theme index), life-operating-calendar (fully styled, red-flag alert + copy/save/clear), year-operating-wheel (month navigator + selection + copy), water-without-fear (public, PDF links), search (query + mobile + result links), 404 (nav + search link). All at 390/430/768/1366px.

Preserved: Water Wonder, all 3 PDFs, Book Tools 001–010 (10 modules), Wisdom, Life Calendar, Year Wheel, Search page, both emails (index.html#contact). No password / invite gate present (verified no access-pass/invite-pass/SITE_ACCESS_PASS/SITE_INVITE_PASS/acl-gate/localStorage-gate in any HTML/JS/CSS).

Safety checks passed: the four blocked terms absent from every file; no medical-diagnosis/treatment/replace-doctor claims (the educational disclaimers on Water Wonder, Life Calendar, and Year Wheel are intact); no guaranteed career outcomes or certification/mastery claims; no confidential company data. All local links and anchors resolve; sitemap valid; robots.txt valid.

Files added: none. Files removed: none. 50 files in → 50 out (pure edits to nav blocks + two lines in search.html).


## v39 — Static Site Search

- Added `search.html` as a static, client-side site search page.
- Added `assets/js/site-search.js` with a generated local search index for Home, Career-OS, Book Tools, Wisdom, Life Calendar, Year Wheel, and Water Wonder.
- Added search styles to `assets/css/mobile-ux.css`.
- Added `Search` to the unified English top navigation across all main pages.
- Updated `sitemap.xml` with the search page.
- Preserved static HTML/CSS/JS only; no backend, no tracking, no password gate, and no external libraries.

## v38 — Mobile "Menu" button hotfix (surgical; no password)

Scope: a surgical mobile-menu fix only, from v37. No password/gate, no redesign, no rebuild, no page-content changes, no change to the unified English nav items (still Home · Career-OS · Book Tools · Wisdom · Life Calendar · Year Wheel · Water Wonder · Contact — byte-identical across all pages, MD5 unchanged from v37).

Root cause (diagnosed in-browser, not assumed):
The nav markup and JS logic were actually present, but two structural fragilities made the mobile menu unreliable on real deployments:
1. Nav visibility for life-operating-calendar.html and year-operating-wheel.html came ONLY from the external styles.css. If that one file failed to load on a clean/sub-path URL, those pages fell back to a raw bulleted <ul> with no toggle styling and no collapse — the "raw bullets / button does nothing" symptom. (Reproduced: with styles.css unreachable, both rendered display:block + list-style:disc even on desktop.)
2. The mobile-nav script ran as a bare IIFE with no DOM-ready guard and no double-bind guard, and asset paths were inconsistent (./assets/ vs assets/ vs none on index/water-without-fear, which carried their own inline copy). Order-dependent and brittle.

Fixes:
- assets/js/main.js — mobile nav rewritten as initMobileNav(): runs on DOMContentLoaded if still loading (never before the DOM exists), and is made idempotent via navToggle.dataset.navBound so the toggle can never receive two click listeners (which would open-then-close on one tap). Selectors standardized to button.nav-toggle / ul#nav-menu. Behavior preserved: click toggles, aria-expanded flips true/false, link-click closes, Escape closes and refocuses the button.
- index.html and water-without-fear.html — these have page-specific inline scripts (sliders/demos) and do NOT load main.js, so their inline copy of the OLD nav logic was replaced in place with the SAME hardened, DOM-ready-safe, idempotent version. No duplicate/conflicting handler now exists anywhere (each page has exactly one nav handler source; the dataset guard prevents double-bind even if that ever changed).
- assets/css/mobile-ux.css — appended a mobile-menu CSS safety patch inside @media (max-width:920px): forces .nav-toggle visible and defines #nav-menu.nav-menu collapsed-by-default with .open => display:flex !important, list-style:none, 44px tap targets, absolute dropdown panel. Because mobile-ux.css is loaded by ALL pages and after each page's embedded <style>, this guarantees a working, consistent collapsed menu even if styles.css ever fails to load — closing fragility #1. !important ensures no earlier embedded rule can leave the menu visible (which made the button look dead).

Asset paths: standardized to consistent relative form (assets/...) on every page. The site deploys flat (pages and the assets/ folder share the root), so relative paths resolve correctly on file://, on clean URLs, and on sub-path hosting alike. (Root-relative /assets/ was trialled and rejected: it breaks file:// and sub-path previews, reintroducing the raw-nav fallback — the opposite of the goal.)

Files changed: assets/js/main.js, assets/css/mobile-ux.css, index.html, career-os.html, wisdom-of-day.html, life-operating-calendar.html, year-operating-wheel.html, water-without-fear.html, 404.html (path normalization on all; inline-nav replacement on index + water-without-fear; nav logic in main.js; CSS patch in mobile-ux.css). Files added: none. Files removed: none. 48 files in → 48 out.

Pages tested (headless Chromium over HTTP, mirroring deployment) at 390 / 430 / 768 / 1366px — index, career-os, wisdom-of-day, life-operating-calendar, year-operating-wheel, water-without-fear, 404: Menu button visible on mobile; tap opens (display:flex, aria-expanded=true); tap again closes (display:none, aria-expanded=false); nav-link click closes; Escape closes; no raw bullets (list-style:none); no unstyled default button; 0px horizontal overflow; desktop nav inline with toggle hidden at 1366px; no console JS errors (the only network 403 is the external Google Fonts request blocked inside the test sandbox; it loads normally on the live domain and the font stack degrades gracefully).

No password / invite gate added or present (verified: no access-pass/invite-pass/SITE_ACCESS_PASS/acl-gate artifacts anywhere). Preserved: unified English nav, Water Wonder, 3 PDFs, Book Tools 001–010, Wisdom, Life Calendar, Year Wheel, both emails (index.html#contact). Forbidden terms absent from every file. GitHub/Vercel ready.


## v37 — Unified top navigation (English, surgical; no password)

Scope: a surgical navigation fix only, starting from the stable v36. No password/gate of any kind, no rebuild, no redesign, no content changes, no new content, no architecture change. The only edits are the top-nav menu markup and the mobile toggle label.

Problem fixed: every page shipped a different top menu — different items, different order, mixed Arabic/English labels, stale anchors (Hydrology, Chemistry, Toolbox, Sources, Modules, Evidence, Smart Fertigation, etc.), and 404 had no menu at all. Now all pages share one identical English menu.

Unified menu (same items, order, and language on every page):
Home → index.html · Career-OS → career-os.html · Book Tools → career-os.html#book-derived-tools · Wisdom → wisdom-of-day.html · Life Calendar → life-operating-calendar.html · Year Wheel → year-operating-wheel.html · Water Wonder → water-without-fear.html · Contact → index.html#contact

Files changed (navigation only):
- index.html — replaced 14-item mixed AR/EN menu with the unified 8-item English menu; toggle label القائمة → Menu.
- career-os.html — replaced the long 14-item menu (Toolbox/Hydrology/Chemistry/Saving Toolkit/Cost Engine/Soft-Hard Skills/Evidence/Modules/Sources) with the unified menu; toggle → Menu.
- wisdom-of-day.html — replaced 8-item mixed menu with the unified menu; toggle → Menu.
- life-operating-calendar.html — replaced 8-item mixed menu with the unified menu; toggle → Menu.
- year-operating-wheel.html — replaced 6-item mixed menu with the unified menu; toggle → Menu.
- water-without-fear.html — replaced 6-item mixed menu (Water Data/Hydrology Signals/المشاريع/التواصل) with the unified menu; toggle → Menu. Article content untouched.
- 404.html — had only a brand and no menu; added the unified menu + the standard styled "Menu" toggle, and loaded main.js + mobile-ux.js so the toggle and current-page highlight work (these were absent on 404 only).

Verified unchanged (not navigation):
- All Arabic page content, headings, and copy — only top-nav labels were translated.
- Book Tools 001–010 content (10 articles intact); the only Book-Tools change is the nav link, as instructed.
- Water Wonder content, all PDFs (3), both emails (index.html#contact), the site identity, embedded per-page CSS, and the existing self-contained asset architecture.

Mobile behavior: identical everywhere. The shared main.js drives the toggle (.nav-menu hidden → .open on tap, aria-expanded synced, Escape closes); on desktop the menu is inline. The menu uses list-style:none with pill links — no raw bullets, no unstyled default button. aria-current="page" (from mobile-ux.js) still highlights the active page because the unified links use real filenames.

Asset loading: left as-is. Paths already resolve consistently for a flat GitHub/Vercel deploy ("assets/..." and "./assets/..." are equivalent at the site root); the brief said not to over-edit pages that already work, so no path churn was introduced. The only asset-link addition is main.js/mobile-ux.js on 404 (previously missing), required for its new toggle.

Safety checks passed: 0px horizontal overflow on all 7 pages at 390 / 430 / 768 / 1366px; nav byte-identical across pages (same MD5, 8 items each); all inline + external JS pass node --check; all local links and #anchors resolve (incl. Book Tools → #book-derived-tools); sitemap.xml valid; emails and PDFs preserved; the four blocked terms absent from every file; no password/gate artifacts present anywhere.

Files added: none. Files removed: none. (48 files in, 48 files out — pure edits.)

Note: this version intentionally contains no access gate. If casual reading-access control is ever wanted, that is a separate feature; if real protection is needed, use hosting-level auth (e.g., Vercel password protection), not static JavaScript.
